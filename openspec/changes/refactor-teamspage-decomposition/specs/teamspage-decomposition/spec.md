...
### Requirement: Acceptance Evidence Via Automated Tests

Every scenario in this spec MUST be verified by a real, executed Vitest + React Testing Library run (`npm run test --prefix Club12-WebClient`), not by a successful build alone. A green `npm run build` MUST NOT be accepted as evidence for any scenario below. Each of the five flow groups (filter/debounce, pagination, create, edit, delete) MUST have at least one passing RTL test exercising its GIVEN/WHEN/THEN, run against both the pre-decomposition monolith (baseline) and the post-decomposition container + presentational tree, using the exact same test file.

#### Scenario: Characterization suite passes against the monolith baseline

- GIVEN the characterization test suite covering all 5 flow groups
- WHEN it is run via `npm run test --prefix Club12-WebClient` against the pre-decomposition `TeamsPage.tsx`
- THEN all tests pass, establishing the behavioral baseline

#### Scenario: Same suite passes unmodified against the decomposed tree

- GIVEN the same characterization test suite, unmodified
- WHEN it is run via `npm run test --prefix Club12-WebClient` against the decomposed container + children
- THEN all tests pass with no scenario skipped, weakened, or replaced by a build-only check
...
### Requirement: Two-PR Delivery Split Leaves `develop` Green At Each Step

PR 1 MUST add `teams.types.ts` and the presentational components (`TeamsFilterBar`, `TeamsTable`, `TeamFormDialog`) as standalone modules with their own passing RTL tests, and MUST leave `TeamsPage.tsx` completely unmodified. PR 1 MUST result in `develop` building and testing green, with the new components tested but not yet imported by any container. PR 2 MUST wire the container to consume these components, delete the duplicated JSX, and MUST leave the full characterization suite from the "Acceptance Evidence" requirement passing unchanged.

#### Scenario: PR 1 lands with new components untouched by the container

- GIVEN PR 1 is merged to `develop`
- WHEN `npm run build --prefix Club12-WebClient` and `npm run test --prefix Club12-WebClient` run
- THEN both succeed
- AND `TeamsPage.tsx` is unchanged from its pre-PR-1 state
- AND the new presentational components have passing standalone tests but are not imported by any existing view

#### Scenario: PR 2 wires components without breaking the characterization suite
