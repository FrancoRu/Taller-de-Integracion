...
namespace Services.Services.Matches.Implementation;
...
public class MatchService(IGenericService<Match> _genericMatchService) : IMatchService
{
    ...
    public async Task GeneratePlayoffMatchesAsync(Guid divisionId, IEnumerable<Team> teams, List<PlayoffSerie> PlayoffSerie)
    {
        DateTime currentMatchDate = DateTime.UtcNow;

        List<List<Team>> pairedTeams = PairTeams(teams);

        List<Match> playoffMatches = GenerateAllMatches(PlayoffSerie, pairedTeams, divisionId, currentMatchDate);
