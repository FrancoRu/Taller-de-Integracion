...
namespace Services.Services.Players.Implementation;
...
public class PlayerService(IGenericService<Player> _genericPlayerService) : IPlayerService
{
    ...
    public async Task<Player?> GetPlayerByIdAsync(Guid playerId) => await _genericPlayerService.FilterByExpression(player => player.Id == playerId).FirstOrDefaultAsync();
