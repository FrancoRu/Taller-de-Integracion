...
{
    public async Task<bool> ValidateCredentialsAsync(User userEntity, string plainTextPassword)
    {
        return await Task.Run(() => Encrypt.CheckHash(plainTextPassword, userEntity.Password));
    }

    public async Task<User?> GetUserByUserNameAsync(string userName)
    {
        return await _genericUserService.FilterByExpression(user => user.Username.Equals(userName)).FirstOrDefaultAsync();
    }

    public async Task<User?> GetUserByRefreshTokenAsync(string refreshToken)
    {
        return await _genericUserService
