...
/// cross-division cup) and exposes a champions history over finished
/// tournaments.
/// </summary>
public interface IChampionService
{
    /// <summary>
    /// Computes a division's podium. When the division has a playoff, the places
    /// come from the top cup's Final (1st = winner, 2nd = loser) and its
    /// third-place match (3rd); otherwise they come from the top three of the
    /// group-phase standings. Undecided places are null.
    /// </summary>
    ...
    /// <summary>
    /// Computes the podium of every division of a tournament. Each division is
    /// included (with null places when its champion is not yet decided) so the
    /// caller sees the whole tournament at a glance.
    /// </summary>
    ...
    /// <summary>
    /// Returns the champion (1st place) of every division of every FINISHED
    /// tournament, optionally scoped to a single season. Divisions whose champion
    /// is not decided are skipped.
    /// </summary>
