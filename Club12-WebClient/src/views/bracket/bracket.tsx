...
const Bracket1: React.FC = () => {
  ...
  const [teamInputs, setTeamInputs] = useState(Array(4).fill("").map((_, i) => `Team ${i + 1}`));
  const [rounds, setRounds] = useState<{ title: string; seeds: { id: number; teams: { name: string }[] }[] }[]>([]);
  ...
  const handleNumTeamsChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTeamInputs(Array(value).fill("").map((_, i) => `Team ${i + 1}`));
  };
  ...
  const startTournament = () => {
    const newRounds = [
      {
        title: "Round One",
        seeds: teamInputs.reduce((acc, team, index, array) => {
          if (index % 2 === 0) {
            acc.push({ id: index / 2 + 1, teams: [{ name: array[index] }, { name: array[index + 1] }] });
          }
          return acc;
  ...
  return (
    ...
    >
      <Paper elevation={4} sx={{ padding: 3, borderRadius: 3, backgroundColor: theme.palette.background.paper }}>
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 2 }}>
        ...
        <Button variant="contained" color="primary" sx={{ marginTop: 2 }} onClick={startTournament}>
          Crear Bracket
        </Button>
      </Paper>
